  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-6">
      <header>
        ...
        <p className="text-sm text-muted">Tu cuaderno de entrenamiento en vivo</p>
      </header>

      {inProgress ? (
        <Surface className="flex items-center justify-between gap-3 border-accent bg-accent-tint/40">
          <div>
            <p className="text-sm font-semibold text-accent-strong">Entrenamiento en curso</p>
        ...
        <Surface className="flex flex-col gap-3">
          ...
          <p className="text-sm text-muted">Elige un plan y regístralo serie a serie mientras entrenas.</p>
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/planes" className="text-sm font-medium text-brand hover:underline">
            Ver todos
          </Link>
        </div>
        {userPlans.length === 0 ? (
            ...
            {userPlans.slice(0, 3).map((plan) => (
              <li key={plan.id}>
                <Surface className="flex items-center justify-between">
                  ...
                  <Link href={`/planes/${plan.id}`} className="shrink-0 text-sm font-medium text-brand hover:underline">
                    Ver
                  </Link>
                </Surface>
              </li>
            ...
            {recentWorkouts.map((session) => (
              <li key={session.id}>
                <Surface className="flex items-center justify-between">
                  ...
                  <Link href={`/historial/${session.id}`} className="shrink-0 text-sm font-medium text-brand hover:underline">
                    Detalle
                  </Link>
                </Surface>
              </li>
